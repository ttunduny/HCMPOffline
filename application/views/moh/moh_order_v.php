...
for($x=$current_year;$x>=$earliest_year;$x--){
	...
	&nbsp;&nbsp;view orders that are:  <a href="<?php echo site_url('order_management/moh_order_v/pending')?>" class="link">Pending</a>&nbsp;|&nbsp;
	<a href="<?php echo site_url('order_management/moh_order_v/approved')?>" class="link">Approved</a>&nbsp;|&nbsp;
	<a href="<?php echo site_url('order_management/moh_order_v/dispatched')?>" class="link">Dispatched</a>&nbsp;|&nbsp;
	<a href="<?php echo site_url('order_management/moh_order_v/delivered')?>" class="link">Delivered</a>
	</fieldset>
	</div>

<table class="data-table">	
	<?php $val=count($order_list);
	 if($val>0){?>
	<tr>
		<?php 
...
?>
	<tr>
        ...
        $datea= $fechaa->format(' jS \of M Y');
		echo $datea;?></td>
		<td><?php 
		$dateb= $rows->approvalDate;
		if($dateb<>""){
		$fechab = new DateTime($dateb);
        $dateb= $fechab->format(' jS \of M Y');
		echo $dateb;
		}
		else {
			$fechab= NULL;
...
$days=$myClass->getWorkingDays($fecha3,$fechad,0);
		...
		<td><a href="<?php echo site_url('order_management/moh_order_details/'.$rows->id)?>"class="link">View</a></td>
	</tr> 
	<?php
		}
	?>
