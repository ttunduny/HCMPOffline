...
<div class="row-fluid">
		...
		<strong> Enter the S11 if available,select Service point,Commodity you wish to issue then Batch No,issue type and quantity issued and input date issued.</strong>
		
		</p>
		</div>
		<div class="col-md-6" id="">
			<p class="text-danger bg-warning">
				<span class="badge ">NB</span> Available Batch Stock 
				<strong>(Units)</strong> is for a specific batch, Total Balance <strong>(Units)</strong> is the total for the commodity.
				</p>
...
<div class="row-fluid">
	...
	<div style="float: right">
		<button class="save btn btn-success" id="step9"><span class="glyphicon glyphicon-open"></span>Save</button>
	</div>
...
$(document).ready(function() {	
	...
	//step one load all the facility data here
		...
		$(".desc").on('change',function(){
			...
			//alert(commodity_id); 
            var dropdown="<option selected='selected' special_data=''>Select Batch</option>"+stock_data[0];
            ...
            console.log(facility_stock_id+'-'+total_issues_for_this_batch+'-'+total_issues_for_this_batch+'-'+total_stock_bal)        
    ...
    $(this).closest("tr").find(".quantity_issued").val(),'return',''))+total_issues_for_this_batch;
				...
				console.log(stock_data[2]+'-'+total_issues_for_this_batch) 
		        ...
		        $("input[name^=facility_stock_id]").each(function(index, value) {  	
                 ...
                 var total_current_issues=$(this).closest("tr").find(".quantity_issued").val();              
                  if($(this).val()==facility_stock_id_current && row_id_<row_id){
				    ...
				    $('.facility_stock_id').each(function() {
				
				        
				        arr[i++] =$(this).closest('tr').find('.facility_stock_id').val();
				        arr_bal[i++] =$(this).closest('tr').find('.available_stock').val();
				    ...
				    $(arr).each(function() {
				      ...
				      var compare=$('#facility_issues_table tr:last').find('.facility_stock_id').val();
				 
				     if(compare==arr[j-1]){
				     $('#facility_issues_table tr:last').find('.available_stock').val(arr_bal[j]);
					alert(arr_bal[j]) ;
					alert($('#facility_issues_table tr:last').find('.available_stock').val());
				     }else{
   ...
   // total_issues=parseInt(calculate_actual_stock(data_array[3],$(this).closest("tr").find(".commodity_unit_of_issue").val(),total_current_issues,'return',''))+total_issues;                 
                  }                
		        });
       ...
       $(".quantity_issued").on('keyup',function (){
        	var av_stock=parseInt($(this).closest("tr").find(".available_stock").val());
        	var com_balance=parseInt($(this).closest("tr").find(".commodity_balance").val());
        	
        	var selector_object=$(this);
        	var data =$('option:selected', selector_object.closest("tr").find('.desc')).attr('special_data') 
	       	var data_array=data.split("^");
	       	
        	var remainder1=com_balance-parseInt(calculate_actual_stock(data_array[3],selector_object.closest("tr").find(".commodity_unit_of_issue").val(),
    selector_object.val(),'return',selector_object));
    console.log('total bal-'+remainder1)
    
    var issue=parseInt(calculate_actual_stock(data_array[3],selector_object.closest("tr").find(".commodity_unit_of_issue").val(),
    selector_object.val(),'return',selector_object));
    console.log('total issues-'+issue)
    
    var remainder=av_stock-parseInt(calculate_actual_stock(data_array[3],selector_object.closest("tr").find(".commodity_unit_of_issue").val(),
    selector_object.val(),'return',selector_object));
     console.log('batch_bal-'+remainder)
    ...
    return;   }// set the balance here
   	...
   	selector_object.closest("tr").find(".available_stock").val(remainder)
        });
