	...
	for($x=0; $x<count($records);$x++)
		{
			...
			$Beginning_Balance[$x] = $records[$x]['Beginning_Balance']; 
			$kemsa_code[$x] = $records[$x]['Kemsa_Code']; 
			$Quantity_Received[$x] = $records[$x]['Quantity_Received']; 
			$Quantity_Dispensed[$x] = $records[$x]['Quantity_Dispensed'];
			$Losses_Excluding_Expiries[$x] = $records[$x]['Losses_Excluding_Expiries'];
			$Positive_Adjustments[$x] = $records[$x]['Positive_Adjustments'];
			$Negative_Adjustments[$x] = $records[$x]['Negative_Adjustments'];
			$Physical_Count[$x] = $records[$x]['Physical_Count'];
			$Expired_Drugs[$x] = $records[$x]['Expired_Drugs'];
			$Days_Out_Stock[$x] = $records[$x]['Days_Out_Stock'];
			$Report_Total[$x] = $records[$x]['Report_Total'];
			$Report_Date[$x] = $records[$x]['Report_Date'];
		...
		<td>".$drugname[$x]."</td>
		<td>
			<input type='text' class='form-control input-small Beginning_Balance' name='Beginning_Balance[".$counter."]' value = '".$Beginning_Balance[$x]."'/>
		</td>
	    <td>
	    	<input type='text' class='form-control input-small Quantity_Received' name='Quantity_Received[".$counter."]'' value = '".$Quantity_Received[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Quantity_Dispensed' name='Quantity_Dispensed[".$counter."]'' value = '".$Quantity_Dispensed[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Losses_Excluding_Expiries' name='Losses_Excluding_Expiries[".$counter."]'' value = '".$Losses_Excluding_Expiries[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Positive_Adjustments' name='Positive_Adjustments[".$counter."]'' value = '".$Positive_Adjustments[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Negative_Adjustments' name='Negative_Adjustments[".$counter."]'' value = '".$Negative_Adjustments[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Physical_Count' name='Physical_Count[".$counter."]'' value = '".$Physical_Count[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Expired_Drugs' name='Expired_Drugs[".$counter."]'' value = '".$Expired_Drugs[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Days_Out_Stock' name='Days_Out_Stock[".$counter."]'' value = '".$Days_Out_Stock[$x]."'/>
	    </td>
	    <td>
	    	<input type='text' class='form-control input-small Report_Total' name='Total[".$counter."]'' value = '".$Report_Total[$x]."'/>
	    </td>
