...
class User_Management extends auto_sms {
	...
	public function index() {
		...
		$data = array();
		$data['title'] = "Login";
		$this -> load -> view("user/login_v", $data);
	}
	...
	public function submit() {
		...
		if ($myvalue == 1) {
			...
			$session_data = array('county_id' => $county_id, 'usertype_id' => $myvalue, 'phone_no' => $phone, 'user_email' => $user_email, 'full_name' => $facility_name, 'user_id' => $user_id, 'user_indicator' => "fac_user", 'names' => $namer, 'inames' => $inames, 'identity' => $id_d, 'news' => $faci, 'district1' => $disto, 'drawing_rights' => $drawing_rights, 'county_name' => $county_name);
		} else if ($myvalue == 3) {
...
public function edit_user_profile() {
		...
		($user_id == $user_id_session) ? $this -> session -> set_userdata(array('phone_no' => $phone, 'user_email' => $email, 'names' => $f_name, 'inames' => $other_name)) : $blank = null;
	...
	public function create_new_facility_user() {
		...
		if ($facility_code != null && $facility_code != 0) {
			...
			$q = Doctrine_Manager::getInstance() -> getCurrentConnection() -> execute('update facilities f, (select  facility, min(`created_at`) 
 ...
 and facility_code=temp.facility');
	...
	public function user_reset() {
		...
		$config = Array('protocol' => 'smtp', 'smtp_host' => 'ssl://smtp.googlemail.com', 'smtp_port' => 465, 'smtp_user' => 'hcmpkenya@gmail.com', // change it to yours
