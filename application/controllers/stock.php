...
class Stock extends MY_Controller {
	...
	public function add_stock_level() {// $facility_code=$this -> session -> userdata('news');  //security check
		if ($this -> input -> post('commodity_id')) :
			...
			foreach ($facility_stock_ as $facility_stock_) :
				...
				$mydata = array('facility_code' => $facility_code, 's11_No' => 'initial stock update', 'commodity_id' => $facility_stock_['commodity_id'], 'batch_no' => "N/A", 'expiry_date' => "N/A", 'balance_as_of' => $facility_stock_['commodity_balance'], 'date_issued' => date('y-m-d'), 'issued_to' => 'N/A', 'issued_by' => $this -> session -> userdata('user_id'));
	...
	public function add_more_stock_level() {//////////////////////////////////ADDING MORE FACILITY STOCK HERE
		if ($this -> input -> post('desc')) :
			...
			for ($i = 0; $i < $count; $i++) :
				...
				// save this infor in the issues table
				$total_unit_count[$i] = $total_unit_count[$i] * -1;
				$mydata = array('facility_code' => $facility_code, 's11_No' => 'stock addition', 'commodity_id' => $commodity_id[$i], 'batch_no' => $batch_no[$i], 'expiry_date' => date('y-m-d', strtotime(str_replace(",", " ", $expiry_date[$i]))), 'balance_as_of' => $facility_stock[0]['commodity_balance'], 'date_issued' => date('y-m-d'), 'issued_to' => 'N/A', 'qty_issued' => $total_unit_count[$i], 'issued_by' => $this -> session -> userdata('user_id'));
				//$this -> session -> userdata('identity')
				// update the issues table
				facility_issues::update_issues_table($mydata);

			endfor;
			$facility_stock = facility_stocks::get_facility_commodity_total($facility_code, null, $date_of_entry) -> toArray();
			foreach ($facility_stock as $facility_stock) :
				//check
				$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection();
				$inserttransaction -> execute("UPDATE `facility_transaction_table` SET `opening_balance` =`opening_balance`+$facility_stock[commodity_balance]
			                                 ,`closing_stock` =`closing_stock`+$facility_stock[commodity_balance]
                                          WHERE `commodity_id`= '$facility_stock[commodity_id]' and status='1' and facility_code=$facility_code");
			endforeach;
			//delete the record from the db
			facility_stocks_temp::delete_facility_temp(null, null, $facility_code);
			//set the notifications
