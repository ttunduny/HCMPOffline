...
public function sendmail($output, $message,$reportname, $email_address) {
    ...
    $emailAttachment = $mpdf->Output('./assets/pdf/'.$reportname . '.pdf', 'S');
    $attach_file = './assets/pdf/' . $reportname . '.pdf';
    if (!write_file('./assets/pdf/' . $reportname . '.pdf', $mpdf->Output('report_name.pdf', 'S'))) {        
        ...
        if ($response) {            
            delete_files('./assets/pdf/' . $reportname . '.pdf');
        }
    }
}
