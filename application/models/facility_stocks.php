...
SELECT 
    ...
    c.unit_size,
    fs.expiry_date,
    ifnull(SUM(ROUND(fs.qty_issued/d.total_commodity_units)),0) AS packs_consumption,
...
/*	echo "SELECT  $selection_for_a_month $computation
	...
	*/
     return $inserttransaction ;
		...
		public static function get_county_consumptionamc_level_new($facility_code, $district_id,$county_id,$category_id,$commodity_id, $option,$from,$to,$graph_type=null){
  	 $selection_for_a_month =((!isset($facility_code) || $facility_code=="ALL") && ($district_id)>0) || $category_id>0 ? " f.facility_name as name," :
  	 (($commodity_id=="ALL") && isset($facility_code) ? " d.commodity_name as name,": 
	 ((isset($county_id) && $district_id=="ALL")? " di.district as name," : 
	($graph_type == 'table_data' && $commodity_id>0) ?" di.district , f.facility_name, f.facility_code, " : 1))  ;
    ...
    CASE temp.selected_option
        ...
        ELSE 0
    END AS amc
    FROM facility_issues fs, facilities f, districts di, commodities d
    LEFT JOIN
    facility_monthly_stock temp ON temp.commodity_id = d.id
    WHERE fs.facility_code = f.facility_code
    AND f.district = di.id
    AND d.tracer_item = '1'
    AND fs.qty_issued >0
...
public static function get_filtered_commodity_consumption_level($facilities_filter,$commodity_filter,$year_filter,$plot_value_filter)
 ...
 public static function get_commodity_consumption_amc($district_id){
  $stocks = Doctrine_Manager::getInstance()->getCurrentConnection()
	->fetchAll("			
SELECT 
	DATE_FORMAT(fs.date_issued,'%d %b %y') as date,
    c.id AS commodity_id,
	fs.facility_code,
    c.commodity_name,
    c.commodity_code,
	ifnull(SUM(ROUND(fs.qty_issued/c.total_commodity_units)),0) AS pack_consumption,
...
WHERE
    di.id = '$district_id' 
    AND fs.date_issued BETWEEN '$from' AND '$to'
    AND fs.commodity_id= ds.commodity_id
    AND c.tracer_item = '1'
    AND c.id = ds.commodity_id
GROUP BY c.id
